...
def get_rescale_sym_constraint_deviation(W1, b, W2, W1_update, b_update, W2_update):
    ...
    """
    return (W1_update.flatten() @ W1.flatten() \
            + b_update.flatten() @ b.flatten() \
            - W2_update.flatten() @ W2.flatten()).item()
...
def get_adam_param_update(param, grad, opter_state, lr=0.001, betas=(0.9, 0.999), eps=1e-8, weight_decay=0.):
    beta1, beta2 = betas
    ...
    param_update = - lr * bias_corrected_first_moment / (torch.sqrt(bias_corrected_second_moment) + eps)
