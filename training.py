...
from utils.others import w, detach_var, rsetattr, rgetattr, count_parameters
from optimizer import Optimizer
from meta_module import *


def do_fit(opter, opter_optim, target_cls, optee_cls, unroll, n_iters, optee_updates_lr,
    train_opter=True, optee_config=None, ckpt_iter_freq=None, ckpt_prefix="", ckpt_dir=""):
    if train_opter:
        opter.train()
    ...
    optee_n_params = sum([int(np.prod(p.size())) for _, p in optee.all_named_parameters()])
    
    ### initialize hidden and cell states
    hidden_states = [w(Variable(torch.zeros(optee_n_params, opter.hidden_sz))) for _ in range(2)]
    cell_states = [w(Variable(torch.zeros(optee_n_params, opter.hidden_sz))) for _ in range(2)]
    ...
    for iteration in range(1, n_iters + 1):
        ...
        result_params = {}
        hidden_states2 = [w(Variable(torch.zeros(optee_n_params, opter.hidden_sz))) for _ in range(2)]
...
def fit_optimizer(target_cls, optee_cls, optee_config=None, preproc=False, unroll=20, n_epochs=20, n_optim_runs_per_epoch=20, n_iters=100,
    n_tests=100, opter_lr=0.001, optee_updates_lr=1.0, ckpt_iter_freq=None, ckpt_prefix="", ckpt_dir=""):
    ...
    for epoch_i in tqdm(range(n_epochs), "epochs"):
        ...
        if test_loss < best_loss:
            print(f"[{epoch_i + 1}/{n_epochs}] New best loss\n\t previous: \t{best_loss}\n\t current:\t {test_loss}")
            best_loss = test_loss
...
def fit_normal(target_cls, optee_cls, opter_cls, n_tests=100, n_iters=100, optee_config=None,
    ckpt_iter_freq=None, ckpt_prefix="", ckpt_dir="", **kwargs):
    if ckpt_iter_freq is not None:
        os.makedirs(ckpt_dir, exist_ok=True)
